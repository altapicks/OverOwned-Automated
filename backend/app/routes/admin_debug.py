...
def watcher_status() -> Dict[str, Any]:
    ...
    try:
        ...
        jobs = [{"error": str(e)}]
    return {"alive": True, "running": getattr(sched, "running", None), "job_count": len(jobs), "jobs": jobs}
    ...
    for lg in tennis_lgs:
        ...
        if status != 200:
            out["per_league"].append({
                "league_id": lid, "league_name": lg["name"], "http": status, "error": err,
                "projections_count": 0, "included_players_count": 0,
                "odds_type_histogram": {}, "stat_histogram": {}, "sample_projections": [],
            })
            continue
        ...
        for i, proj in enumerate(data):
            ...
            if len(sample_projs) < 8:
                ...
                sample_projs.append({
                    ...
                    "stat_type": stat_type, "odds_type": odds_type, "line": attr.get("line_score"),
                })
            if tested_n < 5:
                rel = (proj.get("relationships") or {}).get("new_player") or {}
                pp_pid = str((rel.get("data") or {}).get("id") or "")
        ...
        out["per_league"].append({
            "league_id": lid, "league_name": lg["name"], "http": status,
            "projections_count": len(data), "included_players_count": len(player_idx),
            "odds_type_histogram": dict(odds_hist), "stat_histogram": dict(stat_hist),
...
def pp_lines(slate_id: str = Query(...)) -> Dict[str, Any]:
    ...
    resp = (
        ...
        .eq("slate_id", slate_id).eq("is_active", True)
        .order("raw_player_name").order("stat_type").order("odds_type").execute()
    )
    rows = resp.data or []
    ...
    return {
        "slate_id": slate_id, "row_count": len(rows),
        "by_stat_type": dict(by_stat), "by_odds_type": dict(by_odds),
